...
const setCookie = async (req, res, user) => {
    ...
    res.setHeader('Set-Cookie', [
      `user_meta=${user}; ${req.protocol == 'https'? 'HttpOnly': ''}; Secure; SameSite=None; Path=/; Max-Age=86400`,
      `auth_token=${token}; ${req.protocol == 'https'? 'HttpOnly': ''}; Secure; SameSite=None; Path=/; Max-Age=86400`
    ]);
...
const deleteCookie = async (req, res) => {
  ...
  res.setHeader('Set-Cookie', [
    `user_meta=; ${req.protocol == 'https'? 'HttpOnly': ''}; Secure; SameSite=None; Path=/; Max-Age=0`,
    `auth_token=; ${req.protocol == 'https'? 'HttpOnly': ''}; Secure; SameSite=None; Path=/; Max-Age=0`
  ]);
